...
class Utils {
  ...
  static Uri onlyCode(String url, String langCode) {
    return Uri.parse('$url?').replace(queryParameters: {'lang_code': langCode});
  }
  ...
  //     content: CustomText(
  //       text: Utils.translatedText(context, msg ?? 'Please login first'),
  //       color: textColor,
  //     ),
  ...
  static String formatRelativeTime(String timestamp) {
    ...
    return timeago.format(
      dateTime,
      locale: 'en',
    ); // You can change the locale if needed
  }
  ...
  static String formatDateTime(String timestamp) {
    ...
    String formattedDate = DateFormat(
      'MMM dd yyyy',
    ).format(dateTime); // Example: Mar 03 2024
    String formattedTime = DateFormat(
      'hh:mm a',
    ).format(dateTime); // Example: 09:16 AM
  ...
  //       (c.currencyPosition.toLowerCase() == 'after_price')) {
  //     if (price is double) {
  ...
  static Future<bool?> errorSnackBar(
    BuildContext context,
    String msg, [
